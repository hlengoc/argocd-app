         python -c "
          ...
          def get_pull_requests(org, repo):
              ...
              #     print(pr['number'])
              open_prs = [pr for pr in r.json() if ((pr['created_at'].split('T')[0] == today_str) or (pr['created_at'].split('T')[0] == yesterday_str))  and (pr['state'] == 'open')]
              closed_prs = [pr for pr in r.json() if ((pr['created_at'].split('T')[0] == today_str) or (pr['created_at'].split('T')[0] == yesterday_str)) and (pr['state'] == 'closed')]
              print(len(open_prs))
          ...
          for repo in repos:
              ...
              if num_copilot_comments == 0:
                  ...
                  ##### For closed prs #####
          body += f'************************************************************ LIST OF CLOSED PRs TODAY *******************************************************************************\n\n\n'
          for repo in repos:
              
              closed_prs = get_pull_requests(org, repo)[1]
              if not len(closed_prs):
                  continue
              num_copilot_comments = 0
              pr_body = ''
